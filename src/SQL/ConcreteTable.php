...
class ConcreteTable {
	...
	private /**String*/	$tableName;
        ...
        /**
            It marks a member variable not to be serialized when it is 
         * persisted to streams of bytes. When an object is transferred through the network, 
         * the object needs to be 'serialized'. Serialization converts the object state to 
         * serial bytes. Those bytes are sent over the network and the object is recreated 
         * from those bytes. Member variables marked by the java transient keyword 
	...
	{	
            $this->rowSet = new SplDoublyLinkedList();
            $this->transactionStack	=  new SplDoublyLinkedList();
            $this->tableName   = $tableName;
	    $this->columnNames = array_replace_recursive(array(), $columnNames);
	}
	/**********************************************************************
	 * Return the index of the named column. Throw an
	 * IndexOutOfBoundsException if the column doesn't exist.
	 */
	private function indexOf( /**String */$columnName )
	{	
	    ...
	    throw new OutOfBoundsException("Column (".$columnName.") "
                    . "oesn't exist in " . $this->tableName );
	}
	 ...
	 * an example.
	 */
	public function concreteTableBy( \SQL\Importer $importer )
	{	
            $importer->startTable();

            $tableName =  $importer->loadTableName();
            $width     =  $importer->loadWidth();
            $columns   =  $importer->loadColumnNames();

            $this->columnNames = new SplFixedArray($width);
            
            
            for($i = 0; $columns->valid() ;) {
                $columnNames[$i++] = (String) $columns->current();
                $columns->next();
            }

            while( ($columns = $importer->loadRow()) != null ) {	
                $current = new SplFixedArray($width);
                for($i = 0; $columns->valid() ;) {
                    $current[$i++] = $columns->current();
                    $columns->next();
                }
                $this->insert( $current );
            }
	...
	//----------------------------------------------------------------------
         ...
         * **/
	public function insert( SplFixedArray $values )
	{	
            if ($this->width() != $values->getSize()) {
                throw new \RuntimeException("Values-array length (" . count($values) . ") " .
                    " is not the same as table width (".$this->width().")");
            } else {
                $this->doInsert( clone $values );     
            }
            return 1;
	...
	private function doInsert( /*Object[]*/ SplFixedArray $newRow )
	{
		$this->rowSet->push( $newRow );
		$this->registerInsert( $newRow );
		$this->isDirty = true;
	}
	//@insert-end
	//----------------------------------------------------------------------
	...
	private function registerInsert( /**Object[]**/ SplFixedArray $newRow )
	{	
            if( !$this->transactionStack->isEmpty() ) {
	        $this->register( new UndoInsert($newRow) );
            }
	}
	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	//
         ...
         * @return string
         * **/
	public  function name()				{ return $this->tableName; 		 }
        /**
         * @return void
         * @param string $s
         * **/
	public  function	rename($s)	{ $this->tableName = $s;			 }
	...
	public  function isDirty()			{ return $this->isDirty;			 }
         ...
         * @return integer 
         * **/
	private function	width()				{ return count($this->columnNames); }
	//----------------------------------------------------------------------
...
interface Undo
{
    public function execute(SplDoublyLinkedList $rowSet);
}

class UndoInsert implements Undo
{   
    private final /**Object[]**/ /**@var SplFixedArray */$insertedRow;
    
    public UndoInsert( SplFixedArray $insertedRow )
    {	
    ...
    {	
        $com = $insertedRow->toArray();
        foreach ($this->rowSet as $key => $value) {
            if ($value->toArray() == $com) {
                $this->rowSet->offsetUnset($key);
                break;
            }     
        }
    }
